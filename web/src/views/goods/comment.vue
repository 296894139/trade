...
<script>
import { listComment, deleteComment,listCommentAll } from '@/api/comment'
...
export default {
  ...
  methods: {
    getList() {
      ...
      console.log(this.shopId,88454)
      if(this.shopId==undefined||this.shopId==''){
          listCommentAll(this.listQuery).then(response => {
      ...
      listComment(this.listQuery).then(response => {
        this.list = response.data.data.items
        this.total = response.data.data.total
              console.log('list8',response.data.data.items)
        this.listLoading = false
      }).catch(() => {
        this.list = []
        this.total = 0
