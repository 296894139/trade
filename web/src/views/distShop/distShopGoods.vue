...
<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-button class="filter-item" type="primary" v-if="type!=2" icon="el-icon-edit" @click="handleCreate">添加</el-button>
    ...
    <el-table v-loading="listLoading" :data="list" size="small" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" v-if="type!=2" label="操作" width="200" class-name="small-padding fixed-width">
        <template slot-scope="scope">
              <el-button type="primary" size="mini" @click="handleOpen(scope.row)">上架</el-button>
              <!--
...
export default {
  ...
  data() {
    return {
      list: [],
      total: 0,
      type:0,//用来区分商家和管理员 默认是商家0  管理员为1  
      listLoading: true,
      listQuery: {
        page: 1,
        limit: 20,
  ...
  created() {  
    ...
    console.log(this.roles,999)
    if(this.roles[0]=='超级管理员'){
      this.type= 2
      console.log(this.type,147)
    }
    this.getList()
  },
  methods: {
    getList() {
      ...
      /*if(this.shopId!=undefined||this.shopId!=null){
        listShopGoods(this.shopId,this.listQuery)
      }*/
     ...
     */
      handleOpen(row) {
          ...
          var data = Object.assign({}, row);
             data.picUrl = [];
             data.picUrl.push(row.picUrl);
             data.shopGoodsId = row.id
             data.distributionId= this.distId
             data.content = row.brief
             data.shopGoodsName = row.name

          postData("AdminDistribution/goodsOnSelf",data).then(response=>{
    ...
    showDetail(detail) {
      ...
      window.location.href="http://zb.llgmall.com#/goodsDetail?goodsId="+detail.id;//  http://zb.llgmall.com/#/goodsDetail?goodsId=1181072
