  ...
  export default {
    ...
    data() {
      return {
        ...
        inde: [{'label':'会员','value':'boss'},{'label':'股东','value':'shareholder'},{'label':'员工','value':'staff'},{'label':'庄主','value':'manager'}],
        levels:[],
        shareholderIds:[],
    ...
    methods: {
      getList() {
        ...
        axios({
          method: 'get',
          url: config.baseApi + "user/find/identity?identity=boss&page="+ (this.listQuery.page-1)+"&size=20",
          headers:{
            "X-Litemall-Admin-Token":sessionStorage.getItem('token')
          }
        }).then(response => {
      ...
      changeLevel(){
        ...
        axios({
          method: 'get',
          url: config.baseApi + "user/find/identity-shopId-levelId?identity=boss&shopId="+this.shopId+"&levelId="+levelId+"&page="+ (this.listQuery.page-1)+"&size=20",
          headers:{
            "X-Litemall-Admin-Token":sessionStorage.getItem('token')
          }
        }).then(response => {
      ...
      handleFilter() {
        ...
        axios({
          method: 'get',
          url: config.baseApi + "user/find/identity/query?identity=boss&query="+this.listQuery.key+"&page="+ (this.listQuery.page-1)+"&size=20",
          headers:{
            "X-Litemall-Admin-Token":sessionStorage.getItem('token')
          }
        }).then(response => {
